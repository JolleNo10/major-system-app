...
Cards contains two related card-memory applications: Themed Cards owns one
editable word/person per card, and PAO owns Person/Action/Object triples and
three-card imagery. They share generic card/deck practice mechanics where
their data shapes allow it.

## Entry points

- `index.ts` is the public boundary for app drills and provider composition.
- `themed/ThemedCardsDrill.tsx` and `pao/PaoCardsDrill.tsx` are flavor entries.
...
- `shared/` — generic single-value card and deck-memo mechanics used by Themed;
  PAO keeps its structurally different workflow separate.
...
- Put behavior in `shared/` when it is reusable across Cards flavors without
  flavor conditionals, or when a shared engine keeps a flavor wrapper thin.
...
- Themed deck-memo state uses the storage prefix or history key provided by its
  wrapper. Themed and PAO session behavior must not silently enter global Major
  stats.
...
Outside code imports `ThemedCardsDrill`, `PaoCardsDrill`, and the providers
needed by app composition from `@/features/cards`. Flavor stores,
