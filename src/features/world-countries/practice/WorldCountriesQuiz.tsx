...
export function WorldCountriesQuiz({ answerMode: _answerMode }: { answerMode: AnswerMode }) {
  ...
  if (phase === 'session' && activeRun?.type === 'neighbours') return <NeighboursQuizSession run={activeRun.run} session={activeRun.session} fuzzyMatching={activeRun.run.fuzzyMatching} onSessionChange={updateNeighboursSession} onAdvance={advanceNeighbours} />
