...
describe('Neighbours Quiz session', () => {
  ...
  it('keeps the same session tools available in the expanded map companion', () => {
    ...
    const companion = mount.querySelector<HTMLElement>('[data-map-surface-companion]')
    expect(companion?.querySelector('[data-neighbours-session-tools-variant="compact"]')).not.toBeNull()
    expect(companion?.textContent).toContain('Show number')
    expect(companion?.textContent).toContain('Reveal remaining')
  })
})
