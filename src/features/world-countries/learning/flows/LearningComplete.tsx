...
export function LearningComplete({
  ...
  const regionSummary = regionCompletion && regionLabel
    ? <>
        ...
        <div className="flex flex-wrap gap-x-3 gap-y-1 text-sm"><span>Countries ✓</span><span>Capitals ✓</span><span>Mastery {regionCompletion.masteryStatus === 'mastered' ? 'Mastered' : 'Building'}</span></div>
  ...
  const status = (
    <div className="min-w-0">
      ...
      {completionHandoff && <div className={`${isCompletedRegion ? 'max-w-none' : 'max-w-[360px]'} mt-1 text-sm text-violet-200`}>{completionHandoff.description}</div>}
    </div>
  )
  const dock = (
    <TaskDock variant="completion" contentSizing={isCompletedRegion ? 'contained' : 'intrinsic'} status={status} tone="ready" enableEnterPrimary>
      <div data-completion-actions className={isCompletedRegion ? 'flex w-full flex-wrap justify-end gap-2' : 'flex w-full gap-2 xl:w-auto'}>
        <button type="button" data-primary-action onClick={completionHandoff?.onContinue ?? onDone} className={`${isCompletedRegion ? 'flex-[1_1_12rem]' : 'flex-1'} whitespace-nowrap rounded-[9px] border border-violet-500 bg-violet-600 px-3.5 py-2.5 text-sm font-bold text-white hover:bg-violet-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-400 xl:flex-none`}>{completionHandoff?.label ?? doneLabel}<span aria-label="Enter" className="ml-2 inline-flex min-w-[22px] items-center justify-center rounded-[5px] border border-white/25 border-b-2 px-1.5 py-px text-[11px]">↵</span></button>
        {completionHandoff?.onStop && completionHandoff.stopLabel && <button type="button" data-completion-stop onClick={completionHandoff.onStop} className={`${isCompletedRegion ? 'flex-[1_1_12rem]' : 'flex-1'} whitespace-nowrap rounded-[9px] border border-zinc-600 bg-zinc-800 px-3.5 py-2.5 text-sm font-semibold text-zinc-200 hover:border-violet-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-400 xl:flex-none`}>{completionHandoff.stopLabel}</button>}
