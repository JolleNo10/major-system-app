...
export function LearningComplete({
  ...
  const status = (
    <div className="min-w-0">
      ...
      {completionHandoff && <div className="mt-1 max-w-[360px] text-sm text-cyan-200">{completionHandoff.description}</div>}
  ...
  const dock = (
    <TaskDock variant="completion" status={status} tone="ready" enableEnterPrimary>
      <div className="flex w-full gap-2 xl:w-auto">
        <button type="button" data-primary-action onClick={completionHandoff?.onContinue ?? onDone} className="flex-1 whitespace-nowrap rounded-[9px] border border-cyan-600 bg-cyan-600 px-3.5 py-2.5 text-sm font-bold text-white hover:bg-cyan-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500 xl:flex-none">{completionHandoff?.label ?? doneLabel}<span aria-label="Enter" className="ml-2 inline-flex min-w-[22px] items-center justify-center rounded-[5px] border border-white/25 border-b-2 px-1.5 py-px text-[11px]">↵</span></button>
