...
export function CountryLearningFlow({
  ...
  const context = (() => {
    switch (flow.phase) {
      ...
      case 'location-ready': return <LearningHeader label="Learning context" title={learningScopeLabel} meta={learningSetContext(flow, stageEntries.length)} onExit={onExit} />
      ...
      case 'set-ready': return <LearningHeader label="Learning context" title={learningScopeLabel} meta={learningSetContext(flow, stageEntries.length)} onExit={onExit} />
  ...
  const activeTask: WorldCountriesActivityTask | undefined = (() => {
    switch (flow.phase) {
      case 'walkthrough':
        return { direction: 'Meet the countries', cue: walkthroughCountry?.country ?? 'Country', sessionContext: `Meet the countries · ${learningSetContext(flow, stageEntries.length)}`, progress: { label: 'Country', current: flow.walkthroughIndex + 1, total: stageEntries.length } }
  ...
  switch (flow.phase) {
    ...
    case 'combined-practice':
      ...
      break
    case 'set-ready':
      content = <StagedLearningReadyStep title={getLearningSetCompletionLabel(stagePresentation)} summary={`You recalled all ${stageEntries.length} ${stageEntries.length === 1 ? 'country' : 'countries'} in this practice.`} nextDescription={getNextLearningStageDescription(flow.plan, flow.stageIndex, 'countries')} nextLabel={getNextLearningStageLabel(flow.plan, flow.stageIndex)} onNext={() => run(advanceStagedCountryPlan)} onKeepPractising={() => run(keepStagedCountryPractising)} onBack={() => run(backStagedCountry)} onExit={onExit} celebration="set" surface />
      break
    case 'combined-ready':
      content = <StagedLearningReadyStep title="Mixed practice complete" summary={`You recalled all ${stageEntries.length} introduced countries together.`} nextDescription={getNextLearningStageDescription(flow.plan, flow.stageIndex, 'countries')} nextLabel={getNextLearningStageLabel(flow.plan, flow.stageIndex)} onNext={() => run(advanceStagedCountryPlan)} onKeepPractising={() => run(keepStagedCountryPractising)} onBack={() => run(backStagedCountry)} onExit={onExit} surface />
      break
    case 'final-gate':
...
function learningSetContext(flow: StagedCountryLearningFlowState, count: number): string {
  return getLearningSetContextLabel(deriveLearningStagePresentation(flow.plan, flow.stageIndex), count, 'countries')
}
