...
function peekAnswer(mount: HTMLElement): HTMLElement {
...
describe('MiniSpellingPractice spelling peek', () => {
  ...
  it('uses Cmd on macOS', () => {
    ...
    expect(mount.querySelector('[data-spelling-peek-hint]')?.textContent).toBe('Hold ⌘ to reveal')
  ...
  it('hides on focus loss, document hiding, answer changes, and unmount cleanup', () => {
    ...
    expect(peekAnswer(mount).textContent).toBe('Sweden')
    expect(peekAnswer(mount).hasAttribute('data-spelling-answer-revealed')).toBe(false)
