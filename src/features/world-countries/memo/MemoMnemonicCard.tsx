...
export function MemoMnemonicCard({
  ...
  const stale = countryIds
    ? isSubregionMnemonicStale(savedCountryIds ? { countryIds: savedCountryIds } : null, countryIds)
    : false
      ...
      {loading ? (
        ...
        <div className="mt-3 space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => fileRef.current?.click()} className="rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-1.5 text-xs text-zinc-300 hover:border-cyan-500 hover:text-zinc-100">
              {draftImage ? 'Replace image' : 'Attach image'}
            </button>
            {draftImage && (
              <button type="button" onClick={() => setDraftImage(null)} className="rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-1.5 text-xs text-red-300 hover:border-red-500">
                Remove image
              </button>
            )}
            <button type="button" onClick={() => void save()} className="rounded-lg bg-cyan-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-cyan-500">
              Save
            </button>
            <button type="button" onClick={() => setEditing(false)} className="rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-1.5 text-xs text-zinc-300 hover:text-zinc-100">
              Cancel
            </button>
          </div>
