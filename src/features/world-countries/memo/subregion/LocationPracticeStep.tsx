...
export function LocationPracticeStep({
  ...
  const expected = entries.find(entry => countryId(entry) === location.currentCountryId)
  const feedbackCountry = feedback ? entries.find(entry => countryId(entry) === feedback.expectedId) : null
  const highlightedCountryId = feedbackCountry ? countryId(feedbackCountry) : expected ? countryId(expected) : undefined
      ...
      {feedback && (
        <section className={`rounded-xl border p-4 ${feedback.correct ? 'border-green-500/30 bg-green-500/10' : 'border-red-500/30 bg-red-500/10'}`}>
          <p className={`text-sm font-semibold ${feedback.correct ? 'text-green-300' : 'text-red-300'}`}>
            {feedback.correct ? 'Correct location.' : `That was ${entries.find(entry => countryId(entry) === feedback.expectedId)?.country ?? 'not the target'}.`}
          </p>
        </section>
      )}
    </div>
