...
export function ContinentOrderEditor({
  ...
  const save = () => {
    setContinentSubregionOrder(continent, draft.map(subregion => subregion.id))
    onChanged()
    onClose()
  }
  ...
  return (
    <section className="rounded-xl border border-cyan-500/30 bg-cyan-500/5 p-4" aria-labelledby="continent-order-editor-heading">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h3 id="continent-order-editor-heading" className="font-semibold text-zinc-100">Edit learning order</h3>
      ...
      <p id="continent-order-editor-instructions" className="mt-3 text-xs leading-relaxed text-zinc-500">
      ...
      >
        <ol className="mt-4 space-y-2" aria-describedby="continent-order-editor-instructions">
          {draft.map((subregion, index) => (
            <SortableSubregionRow key={subregion.id} subregion={subregion} index={index} />
          ))}
        </ol>
      </DragDropProvider>
...
function SortableSubregionRow({ subregion, index }: { subregion: SubregionDefinition; index: number }) {
  ...
  return (
    ...
    >
      ...
      <button
        ...
        aria-label={`Reorder ${subregion.label}`}
        aria-describedby="continent-order-editor-instructions"
        title="Drag to reorder"
      >
        ⠿
      </button>
      <span className="min-w-0 flex-1 text-sm text-zinc-300">{subregion.label}</span>
    </li>
  )
}
