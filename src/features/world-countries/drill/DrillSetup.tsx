...
export function DrillSetup({
  ...
  return <>
    ...
    <div className="space-y-3 animate-fade-in">
      ...
      <p className="px-1 text-xs text-zinc-500">{hasProficiencyScope ? `${proficiencyScope.countryIds.length} Countries need work across ${setupContinent ?? 'the World'}, highlighted on the map. Choose Selected regions instead to drill by geography.` : level === 'world' ? 'Select Subregions from the rail, or open a Continent from the rail or map.' : `Selected ${selection.subregionIds.filter(id => subregions.some(subregion => subregion.id === id)).length} of ${subregions.length} Subregions. Hover previews a Subregion; click any Country to select or deselect that Country's Subregion.`}</p>
