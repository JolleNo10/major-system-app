...
import { getContinentsInEffectiveOrder, getCountriesForContinent, getCountriesForSubregion, getSubregionsForContinentInEffectiveOrder } from '@/features/world-countries/geography/queries'
...
export function DrillSetup({
  level, selection, mode, order, purpose, learnPracticeMode, learningStates, hoveredGroupId, onHoverGroup, onSelectionChange, onModeChange, onOrderChange, onPurposeChange, onLearnPracticeModeChange, onStart, onLearnPracticeStart, onWorld, onSelectContinent, onGeographyChanged, entries = countries,
}: {
  ...
  useEffect(() => {
    if (!isDrill && purpose !== 'learn-practise') { setRecallProgress(null); return }
    let active = true
    setRecallProgress(null)
    const progressSkills = isDrill ? skills : ['location-to-country'] as const
    ...
    return () => { active = false }
  }, [entries, isDrill, purpose, skills])
  ...
  return <>
    ...
    <div className="space-y-3 animate-fade-in">
      ...
      <GeographyOverviewMap level={level} continent={level === 'continent' ? selection.continent : undefined} selectedSubregionIds={level === 'continent' ? selection.subregionIds : undefined} countryColorsById={countryColorsById} countryAccessibleDescriptionsById={countryAccessibleDescriptionsById} orderGroups={(level === 'world' ? worldOrder.map(continent => ({ label: continent, countryIds: getCountriesForContinent(continent, entries).map(country => country.id) })) : subregionOrder.map(subregion => ({ label: subregion.label, countryIds: getCountriesForSubregion(selection.continent, subregion.id, entries).map(country => country.id) })))} hoveredGroupId={hoveredGroupId} onHoverGroup={onHoverGroup} onCountryClick={country => { if (editingOrder) return; if (level === 'world') onSelectContinent(country.continent); else toggleSubregion(country.subregionId) }} ariaLabel={level === 'world' ? 'World map for choosing a Continent' : `${selection.continent} map for choosing Subregions`} />
