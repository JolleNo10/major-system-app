...
import { getDrillSessionSkills, getDrillSessionTotalSteps, type DrillSessionState } from './drillSessionState'
...
export function PracticeSessionRails({ selection, proficiencySelection = [], state, onExit, entries, learningStates }: {
  ...
  useRails({
    left: (
      <section className="space-y-4" aria-labelledby="world-countries-practice-context-heading">
        ...
        <div>
          ...
          <h2 id="world-countries-practice-context-heading" className="mt-1 text-lg font-bold text-zinc-100">{selection.continent}</h2>
        </div>
    ...
    rightLabel: 'Session',
  }, [entries, learningStates, onExit, proficiencySelection, progressPercent, selection.continent, selection.subregionIds, state.countryIds.length, state.countryIndex, state.countryOrder.length, state.stepIndex, totalSteps])
  return null
}
