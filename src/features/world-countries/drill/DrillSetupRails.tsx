...
function PurposeSelector({ purpose, groupName, onChange }: { purpose: 'drill' | 'learn-practise'; groupName: string; onChange: (purpose: 'drill' | 'learn-practise') => void }) {
  return <WorldCountriesPanel className="space-y-3" aria-labelledby="world-countries-purpose-heading"><h2 id="world-countries-purpose-heading" className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Purpose</h2>{PURPOSES.map(candidate => <ModeOption key={candidate.id} candidate={candidate} selected={purpose === candidate.id} onSelect={onChange} groupName={groupName} />)}</WorldCountriesPanel>
}
...
function CurrentDrillPanel({ mode, order, groupName, onModeChange, onOrderChange, canStart, onStart }: { mode: WorldCountriesDrillMode; order: WorldCountriesDrillOrder; groupName: string; onModeChange: (mode: WorldCountriesDrillMode) => void; onOrderChange: (order: WorldCountriesDrillOrder) => void; canStart: boolean; onStart: () => void }) {
  return <WorldCountriesPanel aria-labelledby="world-countries-current-drill-heading"><h2 id="world-countries-current-drill-heading" className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Drill mode</h2><div className="mt-4"><fieldset className="space-y-2">{WORLD_COUNTRIES_DRILL_MODES.map(candidate => <ModeOption key={candidate.id} candidate={candidate} selected={candidate.id === mode} onSelect={onModeChange} groupName={groupName} />)}</fieldset></div><div className="mt-4 rounded-xl border border-zinc-800 bg-zinc-950/40 p-3"><div className="flex items-center justify-between gap-3"><h3 className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Drill order</h3><div className="inline-flex rounded-md border border-zinc-800 bg-zinc-950/60 p-0.5" role="radiogroup" aria-label="Drill order"><OrderOption order="ordered" selected={order === 'ordered'} onSelect={onOrderChange}>In order</OrderOption><OrderOption order="random" selected={order === 'random'} onSelect={onOrderChange}>Random</OrderOption></div></div></div><button type="button" disabled={!canStart} onClick={onStart} className="mt-3 w-full rounded-xl bg-cyan-600 px-4 py-3 text-sm font-semibold text-white hover:bg-cyan-500 disabled:cursor-not-allowed disabled:opacity-40">{canStart ? 'Start Drill' : 'Choose a Continent first'}</button></WorldCountriesPanel>
}
