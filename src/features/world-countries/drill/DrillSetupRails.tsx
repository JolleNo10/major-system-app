...
function LearnPracticePanel({ level, selectedCount, selectedStates, countriesIncomplete, mode, proficiencySelected, proficiencyLoading, proficiencyCountryCount, onModeChange, onStart }: { level: 'world' | 'continent'; selectedCount: number; selectedStates: readonly (SubregionLearningState | undefined)[]; countriesIncomplete: boolean; mode: WorldCountriesLearnPracticeMode; proficiencySelected: boolean; proficiencyLoading: boolean; proficiencyCountryCount: number; onModeChange: (mode: WorldCountriesLearnPracticeMode) => void; onStart: (mode: WorldCountriesLearnPracticeMode) => void }) {
  const learning = isWorldCountriesLearningMode(mode)
  const groupName = `world-countries-learn-practice-${useId()}`
  const canStart = level === 'continent' && (proficiencySelected
    ? !proficiencyLoading && proficiencyCountryCount > 0
    : selectedCount > 0)
  const learningGuidance = proficiencySelected && learning
    ? 'Proficiency Learning is temporary: completing it does not mark a Subregion learned.'
    : null
  const noMatching = proficiencySelected && !proficiencyLoading && proficiencyCountryCount === 0
  const disabledLabel = level === 'world'
    ? getSelectionPrompt(level)
    : learningGuidance ?? (noMatching ? 'No Countries currently match the selected proficiency' : proficiencySelected && proficiencyLoading ? 'Loading proficiency…' : getSelectionPrompt(level))
...
function ProficiencyScopePanel({ selection, scope, loading, onChange }: { selection: WorldCountriesProficiencySelection; scope: WorldCountriesProficiencyScope; loading: boolean; onChange: (selection: WorldCountriesProficiencySelection) => void }) {
  const toggle = (filter: WorldCountriesProficiencyFilter) => {
    const next = selection.includes(filter) ? selection.filter(value => value !== filter) : [...selection, filter]
    onChange(next)
  }
