...
describe('DrillSession map presentation', () => {
  it('keeps expanded task context in the dominant card and isolates progress', async () => {
    ...
    expect(taskMain?.querySelector('[data-world-countries-task-cue]')).not.toBeNull()
    expect(taskMain?.querySelector('[data-world-countries-task-context]')?.parentElement).toBe(taskMain)
  ...
  it('requires two consecutive exact spellings in mini practise without advancing', async () => {
    ...
    expect(miniPractice.textContent).toContain('Hold Ctrl')
    const spellingInput = miniPractice.querySelector<HTMLInputElement>('input')!
    expect(miniPractice.querySelector('[data-spelling-peek-answer]')).toBeNull()
