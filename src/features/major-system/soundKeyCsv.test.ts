...
import { parseSoundKeyCsv, serializeSoundKeyCsv, type SoundKeyRow } from '@/features/major-system/soundKeyCsv'
...
describe('parseSoundKeyCsv', () => {
  it('parses quoted fields containing commas and doubled quotes', () => {
    const csv =
      ...
      '0,"s, z",,"""0"" ligner en S",\n' +
      '4,r,,"siste bokstav i ""fouR""",'
    const { rows, errors } = parseSoundKeyCsv(csv)
    expect(errors).toEqual([])
    expect(rows[0].sounds.def).toBe('s, z')
  ...
  it('uses custom over default and allows empty hint', () => {
    const { rows, errors } = parseSoundKeyCsv('7,"k, g","k, g!",,')
    expect(errors).toEqual([])
    expect(rows[0].sounds.custom).toBe('k, g!')
    expect(rows[0].hint.def).toBe('')
  })

  it('collects errors: bad digit, duplicate, missing sounds, wrong column count', () => {
    ...
    expect(joined).toMatch(/expected 5 columns/)
  })

  it('round-trips through serialize (including comma/quote fields)', () => {
    const rows = [
      row('0', ['s, z', ''], ['"0" ligner en S', '']),
      row('7', ['k, g', 'k, g, x'], ['K ser ut som 2 × 7', 'custom, tip']),
    ]
