...
// Sequential chain drill for the digits of π.
// setup → pick how many pairs → study → recall → result

const LEN_KEY = 'major-pi-length'
const MODE_KEY = 'major-pi-studymode'
      ...
      {phase === 'setup' && (
        <div className={`w-full max-w-md space-y-6 p-6 ${panelCls}`}>
          <div className="space-y-3">
            <span className="text-sm font-medium text-zinc-300">How many pairs of π?</span>
            ...
            <p className="text-xs text-zinc-600">
              {length} pairs = first {length * 2} digits of π
            </p>
          </div>
        ...
        <div className="flex flex-col items-center gap-6 w-full max-w-md">
          <div className="flex gap-1.5 items-center flex-wrap justify-center">
            {sequence.map((_, i) => (
              <div key={i} className={`h-1.5 w-8 rounded-full transition-all ${
                i < studyIdx ? 'bg-green-500' : i === studyIdx ? 'bg-cyan-500' : 'bg-zinc-700'
              }`} />
            ))}
          </div>
          ...
          <div className="text-center space-y-1">
            <p className="text-xs text-zinc-600 uppercase tracking-widest">
              Pair {studyIdx + 1} of π
            </p>
            <p className="text-xs text-zinc-700">
              digits {studyIdx * 2 + 1}–{studyIdx * 2 + 2}
            </p>
          </div>
      ...
      {phase === 'recall' && (
        <div className="flex flex-col items-center gap-4 w-full max-w-md">
          ...
          <textarea
            ...
            placeholder="moto, rein, lip …  (space-separated)"
